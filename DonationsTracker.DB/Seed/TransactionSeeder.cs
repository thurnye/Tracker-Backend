...
namespace DonationsTracker.DB.Seed
{
    ...
    {
        public static async Task SeedTransactionsAsync(DonationDbContext context, string userId, List<Category> categories, List<Wallet> wallets)
        {
            ...
            var categoryPool = categories.Select(c => c.Id).ToList();

            var random = new Random();
            var transactions = new List<Transaction>();
            int skipped = 0;

            // Separate wallet pools by type
            var incomeWallets = wallets.Where(w => w.WalletType == "Bank").ToList();
            var expenseWallets = wallets.ToList();

            if (incomeWallets.Count == 0)
            {
                Console.WriteLine("⚠️ No bank wallets found for income transactions. Skipping transaction seed.");
                return;
            }
            ...
            foreach (var raw in rawTransactions)
            {
                try
                {
                    ...
                    var walletId = isCredit
                        ? incomeWallets[random.Next(incomeWallets.Count)].Id
                        : expenseWallets[random.Next(expenseWallets.Count)].Id;
